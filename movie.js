...
returnReviews(APILINKLOCAL);

function returnReviews(url) {
  fetch(url + "movie/" + moiveId)
    .then((res) => res.json())
    .then(function (data) {
      console.log(data);
      data.forEach((review) => {
...
function saveReview(reviewInputId, userInputId, id = "") {
  ...
  if (id) {
    ...
    fetch(APILINKLOCAL + "new", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */* ",
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ user: user, review: review, movieId: moiveId }),
    })
      .then((res) => res.json())
      .then((res) => {
        console.log(res);
